...
CAT_PT = {
    "Drug Metabolism": "Metabolismo de Medicamentos",
    "Methylation": "Metilacao",
    "Neurotransmitters": "Neurotransmissores",
    "Caffeine Response": "Resposta a Cafeina",
    "Cardiovascular": "Cardiovascular",
    "Nutrition": "Nutricao",
    "Fitness": "Aptidao Fisica",
    "Sleep/Circadian": "Sono / Ritmo Circadiano",
    "Inflammation": "Inflamacao",
    "Autoimmune": "Autoimune",
    "Detoxification": "Detoxificacao",
    "Skin": "Pele",
    "Iron Metabolism": "Metabolismo do Ferro",
...
# Tabler icon + semantic color class per category. Lives next to the
# label maps so it's obvious that adding a new category requires both a
# translation AND an icon. Empty color string → accent (blue) inline.
CAT_ICONS = {
    "Drug Metabolism":   ("ti-pill", ""),
    "Methylation":       ("ti-dna", ""),
    "Neurotransmitters": ("ti-brain", ""),
    "Caffeine Response": ("ti-coffee", ""),
    "Cardiovascular":    ("ti-heartbeat", "tr"),
    "Nutrition":         ("ti-salad", "tg"),
    "Fitness":           ("ti-barbell", ""),
    "Sleep/Circadian":   ("ti-moon", ""),
    "Inflammation":      ("ti-flame", "tr"),
    "Autoimmune":        ("ti-shield-x", "tr"),
    "Detoxification":    ("ti-leaf", "tg"),
    "Skin":              ("ti-sparkles", ""),
    "Iron Metabolism":   ("ti-droplet", "tr"),
    "Longevity":         ("ti-hourglass-high", "tg"),
    "Alcohol":           ("ti-glass-full", ""),
}
ZYGOSITY_EN = {
    "AFFECTED": "AFFECTED",
    "CARRIER": "CARRIER",
...
app.jinja_env.globals["CAT_ICONS"] = CAT_ICONS
...
def _retranslate_active_if_needed(active):
    ...
    # using the cached `description_original` (EN) and `description_pt`
    # (Argos output, cached after first translate) — both lookups are
    # dict-swap fast. If we hit a finding that lacks `description_pt` while
    # switching to PT, we fall back to Argos here — but the slow path is
    # supposed to catch that case via _translation_needed → interstitial,
    # so this only runs for stragglers (e.g. findings added after the
    # initial translate).
    ...
    try:
        if _current_lang == "pt":
            ...
            for f in findings:
                ...
                if not src:
                    continue
                if f.get("description_pt"):
                    f["description"] = f["description_pt"]
                else:
                    if tr is None:
...
def _translation_needed(active: dict, target_lang: str) -> bool:
    ...
    Argos is only needed for EN→PT *and* only when we don't already have
    PT translations cached on the findings. Once we've translated a given
    analysis to PT once, subsequent toggles in either direction become
    pure dict swaps:
      - description_original holds the EN source
      - description_pt holds the cached PT translation
      - description is the currently-displayed one
    Both swaps run in microseconds, so we skip the interstitial."""
    if not active:
        return False
    cached = active.get("_lang_cached")
    if cached == target_lang:
        return False
    # PT→EN is always fast (restore from description_original).
    if target_lang != "pt":
        return False
    findings = (active.get("health") or {}).get("findings") or []
    # Need Argos only if at least one finding lacks a cached PT translation.
    return any(f.get("description") and not f.get("description_pt") for f in findings)
    ...
    try:
        ...
        if target_lang == "pt":
            ...
            for i, f in enumerate(findings, start=1):
                ...
                src = f["description_original"]
                if src:
                    # Per-finding PT cache: once we translate a finding's
                    # description, store it on the finding itself so a
                    # subsequent EN→PT toggle is a dict swap, not another
                    # Argos round-trip. The session-local `cache` above
                    # still helps when multiple findings share text (e.g.
                    # boilerplate "Variant of uncertain significance").
                    if f.get("description_pt"):
                        f["description"] = f["description_pt"]
                    else:
                        if src not in cache:
